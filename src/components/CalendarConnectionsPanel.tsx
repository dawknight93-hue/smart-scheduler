      ...
      <div className="w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl border border-slate-700 bg-slate-900 shadow-2xl" onClick={(event) => event.stopPropagation()}>
        ...
        <div className="space-y-4 p-6">
          <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-blue-500/30 bg-blue-500/10 p-4">
            ...
            {connectedEmail ? <button onClick={() => void disconnect()} className="rounded-lg border border-blue-300/30 px-3 py-2 text-xs font-medium text-blue-100 hover:bg-blue-500/20">Disconnect</button> : <button onClick={() => void connectGoogle()} disabled={!isGoogleConfigured()} className="rounded-lg bg-blue-600 px-3 py-2 text-xs font-medium text-white hover:bg-blue-500 disabled:cursor-not-allowed disabled:opacity-50">Connect Google</button>}
          </div>
          ...
          {loading ? <div className="py-10 text-center text-sm text-slate-500">Loading calendar settings…</div> : connections.map((connection) => (
            <div key={connection.id} className="rounded-xl border border-slate-800 bg-slate-950/50 p-4">
              ...
              {connection.last_synced_at && <div className="mt-2 text-[10px] text-slate-500">Last synced {new Date(connection.last_synced_at).toLocaleString()}</div>}
